state("Undertale")
{
	...
	uint inventory : "Undertale.exe", 0x39A148, 0x4, 0x80, 0x4, 0x4;

	double lesserdog : "Undertale.exe", 0x59C2AC, 0x84, 0x140, 0x140, 0x4, 0x7C0;
	...
	double monstertale : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0x1C20;
	double latchkey : "Undertale.exe", 0x39A148, 0x4, 0x4, 0x4, 0x4, 0x1C60;
...
startup
{
	...
	settings.Add("p-handhold", false, "Handhold", "ruins");
	...
	settings.Add("i-pie", false, "Get the B'scotch Pie", "ruins");
	...
	settings.Add("f-lesserdog", false, "Pet Lesser Dog to Max Length", "tundra");
	...
	settings.Add("i-punchcard", false, "Get Punchcard", "waterfall");
	settings.Add("r-onionsan", false, "Exit Onion-san Room", "waterfall");
	settings.Add("p-monsterkid", false, "Climb on Monster Kid's Shoulders", "waterfall");
	...
	settings.Add("i-pan", false, "Get Burnt Pan", "hotlands");
	...
	settings.Add("r-mettaton", false, "Exit Mettaton Room", "hotlands");

	settings.Add("f-longelevator", false, "Exit Long Elevator", "castle");
	settings.Add("f-monstertale", false, "Finish Story Battles", "castle");
	...
	settings.Add("f-undyne", false, "Get Undyne's Letter", "truepac");
	...
	vars.goals = new Dictionary<string, object[]>() {
		// Ruins
		{"p-handhold",		new object[] {false,	30,		7.5,	-1,		10,		-1,		-1	}},
		...
		{"i-pie",			new object[] {false,	30,		-1,		-1,		36,		-1,		1	}},
		...
		{"f-lesserdog",		new object[] {false,	106,	-1,		-1,		-1,		24,		2	}},
		...
		{"i-punchcard",		new object[] {false,	126,	-1,		-1,		96,		-1,		4	}},
		{"r-onionsan",		new object[] {false,	126,	-1,		100,	101,	-1,		-1	}},
		{"p-monsterkid",	new object[] {false,	126,	112,	-1,		109,	-1,		-1	}},
		...
		{"i-pan",			new object[] {false,	199,	-1,		-1,		-1,		-1,		6	}},
		...
		{"r-mettaton",		new object[] {false,	199,	-1,		211,	212,	-1,		9	}},

		// Castle
		{"f-longelevator",	new object[] {false,	-1,		-1,		215,	216,	-1,		9	}},
		{"f-monstertale",	new object[] {false,	-1,		-1,		309,	230,	-1,		10	}},
		...
		// Truepac
		{"f-undyne",		new object[] {false,	-1,		-1,		-1,		-1,		-1,		12	}},
...
init
{
	...
	vars.checkInventory = (Func<int, bool>)((id) =>
	{
		for (int i = 0; i < 8; i++)
			if (memory.ReadValue<double>(new IntPtr(current.inventory + i * 0x10)) == id)
				return true;
...
start
{
	if (old.start != current.start || old.room != current.room)
		vars.log("EVENT  r{" + old.room + "," + current.room + "} s{" + old.start + "," + current.start + "}");
			...
			if (vars.goals[goal][vars.special] != -1)
			{
				...
				switch((int)vars.goals[goal][vars.special])
				{
					...
					case 3:	// f-papyrus
						...
						break;
					case 4:		// i-punchcard
						pass = (vars.checkInventory(26));
						break;
					case 5:		// f-temmie
						pass = (old.money - current.money > 999);
						break;
					case 6:		// i-pan
						pass = (vars.checkInventory(47));
						break;
					case 7:		// b-endrgfight
					...
					case 8:		// b-mettaton
						...
						break;
					case 9:		// r-mettaton && f-longelevator
						pass = (current.latchkey == 0);
						break;
					case 10:	// f-monstertale
						pass = (current.monstertale == 17);
						break;
					case 11:	// f-genoend
						pass = (old.chara == 8) && (current.chara > 8);
						break;
					case 12:	// f-undyne
						pass = (vars.checkInventory(56));
						break;
					case 13:	// f-tpeend
