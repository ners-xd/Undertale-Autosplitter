state("Undertale", "v1.001")
{
	...
	double papyrus : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0x430;
	double chara : "Undertale.exe", 0x59C2AC, 0x80, 0x4, 0x200;
	float handhold : "Undertale.exe", 0x59E470, 0xA0, 0x7C;

	double kpointer : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0xC80;
	double cur_kills : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0xC90;
	double mdrcnt : "Undertale.exe", 0x39A148, 0x4, 0x1E0;
	double kills_ruins : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0xCA0;
	double kills_tundra : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0xCB0;
	double kills_waterfall : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0xCC0;
	double kills_hotland : "Undertale.exe", 0x39A148, 0x4, 0x40, 0x4, 0x4, 0xCD0;

	double status_undyne : "Undertale.exe", 0x0039A148, 0x4, 0x40, 0x4, 0x4, 0x1850;
}
state("Undertale", "v1.001 Linux")
{
	...
	float handhold : "Undertale.exe", 0x59F504, 0x8, 0x7C;

	double kpointer : "Undertale.exe", 0x39B1E8, 0x4, 0x40, 0x4, 0x4, 0xC80;
	double cur_kills : "Undertale.exe", 0x39B1E8, 0x4, 0x40, 0x4, 0x4, 0xC90;
	double mdrcnt : "Undertale.exe", 0x39B1E8, 0x4, 0x1E0;
...
state("Undertale", "v1.05")
{
	...
	double papyrus : "Undertale.exe", 0x40894C, 0x44, 0x10, 0xA0, 0x0, 0x24, 0x4, 0x430;
	double chara : "Undertale.exe", 0x618EDC, 0x80, 0x44, 0x10, 0x34, 0x0;
	float handhold : "Undertale.exe", 0x61B004, 0x8, 0x9C;
	
	double kpointer : "Undertale.exe", 0x40894C, 0x44, 0x10, 0xA0, 0x0, 0x24, 0x4, 0xC80;
	double mdrcnt : "Undertale.exe", 0x40894C, 0x44, 0x10, 0xAC, 0x10;
	...
	double status_undyne : "Undertale.exe", 0x40894C, 0x44, 0x10, 0xA0, 0x0, 0x24, 0x4, 0x1850;
}
state("Undertale", "v1.08")
{
	...
	double chara : "Undertale.exe", 0x618EDC, 0x80, 0x44, 0x10, 0xB8, 0x1A0;
	float handhold : "Undertale.exe", 0x61B004, 0x8, 0x9C;
...
state("Undertale", "v1.08 GOG")
{
	...
	double start : "Undertale.exe", 0x618EDC, 0x80, 0x44, 0x10, 0x5BC, 0x0;
	double papyrus : "Undertale.exe", 0x40894C, 0x44, 0x10, 0x130, 0x0, 0x4, 0x4, 0x430;
	double chara : "Undertale.exe", 0x618EDC, 0x84, 0x44, 0x10, 0xB8, 0xA0;
	float handhold : "Undertale.exe", 0x61B004, 0x8, 0x9C;
...
startup
{
	...
	settings.Add("r-touchdoor", false, "Touch Door After Beating Toriel", "ruins");
	...
	settings.Add("r-spears2", false, "Second Undyne Spears", "waterfall");
	...
	vars.goals = new Dictionary<string, object[]>() {
		...
		// Truepac
		{"r-undynedate",	new object[] {false,	-1,		-1,		117,	116,	-1,		389	}},
		...
		{"g-hotland40",	    new object[] {false,	-1,		-1,		-1,		-1,		-1,		23	}}
	};
...
init
{
	...
	current.kills = 0;
	vars.log("Autosplitter by spaceglace(1.01 and earlier) and antimyt (1.05/1.08). Additional split options added by Luke Saward and deesoff.");
	print("Autosplitter by spaceglace(1.01 and earlier) and antimyt (1.05/1.08). Additional split options added by Luke Saward and deesoff.");
...
update
{
	...
	if(settings["displaykillcount"]) {
		vars.Helper.Texts["kills"].Text1 = "Kills:";
		if(current.kills_ruins >= 20 && current.kills_tundra >= 16 && current.kills_waterfall >= 18 && current.kills_hotland >= 40) {
   		vars.Helper.Texts["kills"].Text2 = "(GENO ACHIEVED) " + current.mdrcnt.ToString();
		}
		else {
    		vars.Helper.Texts["kills"].Text2 = current.mdrcnt.ToString();
		}
	}
	else {
		vars.Helper.Texts.Remove("kills");
	}
	if(settings["displayruinskillcount"]) {
		vars.Helper.Texts["ruinskills"].Text1 = "Ruins Kills:";
		vars.Helper.Texts["ruinskills"].Text2 = current.kills_ruins.ToString() + "/20";
	}
	else {
		vars.Helper.Texts.Remove("ruinskills");
	}
	if(settings["displaysnowdinkillcount"]) {
		vars.Helper.Texts["snowdinkills"].Text1 = "Snowdin Kills:";
		vars.Helper.Texts["snowdinkills"].Text2 = current.kills_tundra.ToString() + "/16";
	}
	else {
		vars.Helper.Texts.Remove("snowdinkills");
	}
	if(settings["displaywaterfallkillcount"]) {
		vars.Helper.Texts["waterfallkills"].Text1 = "Waterfall Kills:";
		vars.Helper.Texts["waterfallkills"].Text2 = current.kills_waterfall.ToString() + "/18";
	}
	else {
		vars.Helper.Texts.Remove("waterfallkills");
	}
	if(settings["displayhotlandkillcount"]) {
		vars.Helper.Texts["hotlandkills"].Text1 = "Hotland/CORE Kills:";
		vars.Helper.Texts["hotlandkills"].Text2 = current.kills_hotland.ToString() + "/40";
	}
	else {
		vars.Helper.Texts.Remove("hotlandkills");
	}
	if(settings["displayareakillcount"]) {
		if(current.kpointer == 202){
			vars.Helper.Texts["areakills"].Text1 = "Ruins Kills:";
			vars.Helper.Texts["areakills"].Text2 = current.kills.ToString() + "/20";
			vars.killAreaSet = true;
		}
		else if(current.kpointer == 203) {
			vars.Helper.Texts["areakills"].Text1 = "Snowdin Kills:";
			vars.Helper.Texts["areakills"].Text2 = current.kills.ToString() + "/16";
			vars.killAreaSet = true;
		}
		else if(current.kpointer == 204) {
			vars.Helper.Texts["areakills"].Text1 = "Waterfall Kills:";
			vars.Helper.Texts["areakills"].Text2 = current.kills.ToString() + "/18";
			vars.killAreaSet = true;
		}
		else if(current.kpointer == 205) {
			vars.Helper.Texts["areakills"].Text1 = "Hotland/CORE Kills:";
			vars.Helper.Texts["areakills"].Text2 = current.kills.ToString() + "/40";
			vars.killAreaSet = true;
		}
		else if(vars.killAreaSet == false) {
			vars.Helper.Texts["areakills"].Text1 = "Determination.";
		}
	}
	else {
		vars.Helper.Texts.Remove("areakills");
	}
		...
		if ((settings[goal]) && (!vars.goals[goal][vars.visited]))
		{
			...
			if (vars.goals[goal][vars.special] != -1)
			{
				...
				switch((int)vars.goals[goal][vars.special])
				{
					...
					// tpe cases
					case 389:
						pass = old.status_undyne == 4 && current.status_undyne == 5 && current.status_undyne != old.status_undyne;
						break;
				}
